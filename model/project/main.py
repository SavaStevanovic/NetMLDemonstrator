...
if th_count==1:
    trainset.ids      = trainset.ids[:1000]
    validationset.ids = validationset.ids[:1000]

trainloader      = torch.utils.data.DataLoader(trainset,      batch_size=1,   shuffle=True,  num_workers=th_count, pin_memory=True)
validationloader = torch.utils.data.DataLoader(validationset, batch_size=1,   shuffle=False, num_workers=th_count, pin_memory=True)
