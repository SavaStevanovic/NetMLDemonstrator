...
class RunningConfusionMatrix():
    ...
    def update_matrix(self, ground_truth, prediction):
        ...
        if (ground_truth == self.ignore_label).all():
            ...
            return
        # ground_truths = np.array_split(ground_truth, os.cpu_count())
        # predictions   = np.array_split(prediction  , os.cpu_count())
        # args = [(ground_truths[i], predictions[i], self.labels) for i in range(len(ground_truths))]
        # if not sys.warnoptions:
        #     warnings.simplefilter("ignore")
        # results = self.pool.starmap(confusion_matrix, args)
