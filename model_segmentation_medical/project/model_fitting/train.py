...
def fit(net, trainloader, validationloader, epochs=1000, lower_learning_period=10):
    ...
    for epoch in range(train_config.epoch, epochs):
        loss, output_images, label_images, focal_loss, dice_loss, accs, f1_score, cm = fit_epoch(
            net, trainloader, train_config.learning_rate, train=True, epoch=epoch)
        writer.add_scalars('Train/Metrics', {'loss': loss, 'focal_loss': focal_loss,
                           'dice_loss': dice_loss, 'accs': accs, 'f1_score': f1_score}, epoch)
        ...
        writer.add_images('train_confusion_matrix',
                          cm, epoch, dataformats='HWC')

        loss, output_images, label_images, focal_loss, dice_loss, accs, f1_score, cm = fit_epoch(
            net, validationloader, None, train=False, epoch=epoch)
        writer.add_scalars('Validation/Metrics', {'loss': loss, 'focal_loss': focal_loss,
                           'dice_loss': dice_loss, 'accs': accs, 'f1_score': f1_score}, epoch)
        grid = join_images(label_images)
        ...
        writer.add_images('validation_confusion_matrix',
                          cm, epoch, dataformats='HWC')
